...
public class RegisterPage extends BasePage {
	...
	private final By validationMessage =By.cssSelector("span.field-validation-error");
...
//	}
	...
	@Step("Check validation error message")
	public boolean isValidationErrorDisplayed(String expectedMessage) {

		logger.debug("Validating error message: {}", expectedMessage);

		try {

			return waitForVisibility(validationMessage, SHORT_TIMEOUT).getText().contains(expectedMessage);

		} catch (Exception e) {

			logger.error("Validation message not displayed: {}", expectedMessage);
	...
	protected void validatePageLoaded() {
		...
		if (!getDriver().getCurrentUrl().contains(REGISTER_PATH)) {
			...
			throw new IllegalStateException(
					"Registration page failed to load. Current URL: " + getDriver().getCurrentUrl());
		}

		if (!isElementDisplayed(registerButton, DEFAULT_TIMEOUT)) {

			throw new IllegalStateException("Register button is not visible on registration page.");
		}
	}
